...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  closeTab: (projectId, fileId) => {
    ...
    let newActiveTabsId = activeTabId;
    if (activeTabId === fileId) {
      if (newTabs.length === 0) newActiveTabsId = null;
      else if (tabIndex >= newTabs.length) {
        newActiveTabsId = newTabs[newTabs.length - 1];
      } else {
        newActiveTabsId = newTabs[tabIndex];
      }
    }
    tabs.set(projectId, {
      openTabs: newTabs,
      activeTabId: newActiveTabsId,
